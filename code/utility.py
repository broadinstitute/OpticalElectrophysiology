...
def linear_regression(X, Y=None, order=None, X_test=None):
    ...
    if Y is None:
        if order is None:
            ...
            for k in [k for k, v in sorted(locals().items()) if isinstance(v, torch.Tensor) and k not in ['beta_left']]:
                del locals()[k]
            torch.cuda.empty_cache()
            return beta_left
        else:
    ...
    if order is None:
        ...
        for k in [k for k, v in sorted(locals().items()) if isinstance(v, torch.Tensor) and k not in ['beta', 'Y_test']]:
            del locals()[k]
        torch.cuda.empty_cache()
        return beta, Y_test
    else:
...
#     for k in [k for k, v in sorted(locals().items()) if isinstance(v, torch.Tensor) and k not in ['cor']]:
#         del locals()[k]
#     torch.cuda.empty_cache()
    return cor


def neighbor_cor(mat, neighbors=8, plot=True, choice='max', title='Correlation Map', return_adj_list=False):
    ...
    if return_adj_list:
        adj_list = np.concatenate([get_adj_list(c, nrow, ncol, i+1) for i, c in enumerate([cor1, cor2, cor3, cor4])], axis=0)
        return adj_list
...
def get_label_image(image, min_pixels=50, square_width=3, thresh=None, label_image=None, split_label=None, split_label_segmentation=None, 
    ...
    if split_label is not None and split_label_segmentation is not None:
        regions = regionprops(label_image)
        minr, minc, maxr, maxc = regions[split_label].bbox
        for i, l in enumerate(np.unique(split_label_segmentation)):
            sub_label_image = label_image[minr:maxr, minc:maxc]
            sub_label_image[np.logical_and(sub_label_image>0, split_label_segmentation==l)] = split_label+1 if i == 0 else label_image.max()+1
...
def get_seg(diff, topk=100, min_dist=500, verbose=True):
