...
def detrend_linear(mat, train_idx, linear_order=3, input_min=-2, input_max=2, return_trend=False, device=torch.device('cuda')):
    input_aug = torch.linspace(input_min, input_max, mat.shape[0], device=device)
    beta, trend = linear_regression(X=input_aug[train_idx], Y=mat.reshape(mat.shape[0], -1)[train_idx], order=linear_order, X_test=input_aug)
    mat_adj = mat - trend.reshape(mat.shape)
    if return_trend:
...
def extract_super_pixels(mat_adj=None, test_left=None, test_right=None, mat_cat=None, num_neighbors=8, cor_choice='mean', connectivity=None, 
    ...
    if image is None:
        ...
        cor_global = neighbor_cor(mat_cat, neighbors=num_neighbors, plot=plot, choice=cor_choice, title='correlation map')
        if use_mean_image:
...
def get_submat_traces(regions, label_image, seg_idx=0, mat_adj=None, sig_list=None, mat_list=None, mat=None, cor=None,
    ...
    """
    def get_trace(mat=None, mat_adj=None, seg_idx=None, submat=None, weight_percentile=weight_percentile, plot_singular_values=plot_singular_values):
        if submat is None:
            if mat is None:
                mat = mat_adj[seg_idx]
